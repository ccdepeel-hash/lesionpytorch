# buildmodel_leaves is intended to create dataset for leaf segmentation, rather than lesion segmentation.
...
# PATHING
# Separate folder for leaf detection training data
image_dir = "~/kliebengrp/input_leaves/images"
mask_dir = "~/kliebengrp/input_leaves/masks"

# Ensure directories expand correctly (important for "~")
image_dir = os.path.expanduser(image_dir)
mask_dir = os.path.expanduser(mask_dir)

# Image size - same as lesion detection for consistency
img_size = (277, 394)
...
dataset = LeafDataset(image_dir, mask_dir, img_size=img_size, augment=True)
...
# Adjust classifier for single output class (leaf vs background)
...
# Model attempting to predict segmentation mask (leaf shape) from image
...
# Separate checkpoint folder for leaf detection model
os.makedirs("checkpoints_leaves", exist_ok=True)
torch.save(model.state_dict(), "checkpoints_leaves/leaf_model_init.pth")
print("Initial leaf detection model saved successfully.")

# make objects importable by other scripts
# runmodel_leaves.py imports these to continue training
