# predit_leaves_large.py
# Predicts LEAVES on large images (e.g., 2400x4300) using sliding window approach
import torch
...
output_dir = os.path.expanduser("~/kliebengrp/predicted_masks_leaves_large")  # output folder
checkpoint_path = os.path.expanduser("~/kliebengrp/checkpoints_leaves/leaf_latest.pth")
...
print(f"Loading trained leaf detection model from {checkpoint_path}")
...
with torch.no_grad():
    for filename in image_files:
        ...
        # Overlay
        axes[2].imshow(image)
        axes[2].imshow(mask_binary, cmap='Greens', alpha=0.5)  # Green for leaves instead of red
        axes[2].set_title("Overlay (Green = Leaves)")
        axes[2].axis("off")
